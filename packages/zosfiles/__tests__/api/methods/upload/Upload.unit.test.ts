...
describe("z/OS Files - Upload", () => {
    ...
    describe("dirToUSS", () => {
        const isDirSpy = jest.spyOn(IO, "isDir");
        const ussSetSpy = jest.spyOn(Upload as any, "directoryToUSS");
        const lsStatSpy = jest.spyOn(fs, "lstat");
        ...
        it("should throw error if directory path is not specified", async () => {
            try {
                response = await Upload.directoryToUSS(dummySession, undefined, dsName);
            } catch (err) {
                error = err;
            }
        ...
        it("should throw error if USS base name is not specified", async () => {
            const testDir = "path/to/a/file";
            try {
                response = await Upload.directoryToUSS(dummySession, testDir, undefined);
            } catch (err) {
                error = err;
            }
            ...
            expect(response).toBeUndefined();
            expect(error).toBeDefined();
            expect(error.message).toContain(ZosFilesMessages.missingUssDirectory.message);
        });
        ...
        it("return with proper message when path is pointing to a file", async () => {
            ...
            try {
                response = await Upload.directoryToUSS(dummySession, testPath, dsName);
            } catch (err) {
                error = err;
            }
            ...
            expect(response).toBeUndefined();
            expect(error).toBeDefined();
            expect(error.message).toContain(ZosFilesMessages.emptyLocalDirectory.message);
        });
        it("return with proper response", async () => {
            const testReturn = {};
            const testPath = "test/path";
            isDirSpy.mockReturnValueOnce(true);
            ussSetSpy.mockReturnValueOnce(testReturn);
            lsStatSpy.mockImplementationOnce((somePath, callback) => {
                callback(null, { isFile: () => false });
            });

            try {
                response = await Upload.directoryToUSS(dummySession, testPath, ussName, true);
            } catch (err) {
                error = err;
            }
            ...
            expect(error).toBeUndefined();
            expect(response).toBeDefined();
            expect(ussSetSpy).toHaveBeenCalledTimes(1);
            expect(ussSetSpy).toHaveBeenLastCalledWith(dummySession, testPath, ussName, true);
        });
    });
});
